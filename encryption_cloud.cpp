/*
*Meng Liu
*CSE664 Professor Sheng Zhong
*Encryption Medical Cloud: Server Code
*/
...
int string_compare(char *user_pwd,char *password)
{
	while (((*user_pwd)!='\0')&&((*password)!='\0'))
	{
...
void pro_user_login(int client_sockfd)  // user login, check the id and password
{
	...
	printf("username:%s\n", pac_user_login->user_name);
	printf("password:%s\n", pac_user_login->user_pwd);
	...
	else							// log in
	{
		...
		pac_server_login_ack = (server_login_ack *) buf;
		compare = string_compare(pac_user_login->user_pwd,password);
		if(compare)
		{
...
void pro_user_choice(int client_sockfd)
{
	...
	char message[MAX_URL_LEN] = "http://www.cse.buffalo.edu/UBMM/data/uploads/mengliu/";
	//char data_path[64]=
	pac_user_choice = (user_choice *) buf;
	strcpy(g_choice_image,pac_user_choice->choice_image);
	
	//strcpy(pac_server_push_data->data_packet,data_path);
	strcat(message,g_choice_database);
	strcat(message,g_choice_image);
	strcat(message,".bmp");
	...
	strcpy(pac_server_push_data->data_packet, encryptedText.c_str());
...
{
  int nread;
  int packet_type_int = packet_type - '0';
 ...
 main(int argc, char *argv[])
{
  ...
  strcpy(priKey, "pri"); // private key

  strcpy(pubKey, "pub"); // public key
  ...
  if (argc == 1)
  {
	  ...
	  printf("New Key Generated, please updated!\n");
	  return;
  }
  else if(argc != 2) {
    printf("Usage:\n Run server: ./bserver <PORT>\n Generate New Key: ./bserver\n");
    exit(2);
  }
  //load_database();
